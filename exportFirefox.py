    ...
    def decrypt_passwords(self, profile, password, export):
        ...
        for host, user, passw, enctype in credentials:
            ...
            if export:
                ...
                arrayHosts.append(host)
                arrayUsers.append(user)
                arrayPasswords.append(passw)

        longestHostLength = len(max(arrayHosts, key=len))
        longestUserLength = len(max(arrayUsers, key=len))

        for host, user, passw in zip(arrayHosts, arrayUsers, arrayPasswords):
            newline = "{:<" + str(longestHostLength) + "}  {:<" + str(longestUserLength) + "}  {:<12}\n\r"
            newLine = newline.format(host.encode("utf8"), user.encode("utf8"), passw.encode("utf8"))
            file.write(newLine)
